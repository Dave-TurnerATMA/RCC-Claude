...
export default function Volunteer() {
  ...
  const [createForm, setCreateForm] = useState({ short_description: '', overview: '', priority: 'medium', scheduled_date: '', estimate_hours: '', responsible_user_id: '' });
  ...
  const handleCreate = async () => {
    if (!createForm.short_description || !createForm.overview) return;
    setSaving(true);
    try {
      ...
      setShowCreate(false);
      setCreateForm({ short_description: '', overview: '', priority: 'medium', scheduled_date: '', estimate_hours: '', responsible_user_id: '' });
      load();
    } catch (e: any) {
      alert(e.message);
    } finally {
    ...
    <div className="flex flex-col">
      ...
      <div className="bg-white border-b sticky top-16 z-20 px-4 py-3 space-y-3">
        <div className="flex items-center justify-between">
          <h1 className="font-bold text-gray-900 text-lg">{t('nav.volunteer')}</h1>
      ...
      <div className="flex-1 p-3 space-y-3">
        {loading ? (
          ...
          <VolunteerCard key={task.id} task={task} isOverdue={isOverdue(task)}
          ...
          <div className="relative bg-white w-full max-w-lg rounded-t-2xl sm:rounded-2xl shadow-2xl max-h-[85vh] overflow-y-auto">
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <input type="text" value={createForm.short_description} onChange={e => setCreateForm(p => ({ ...p, short_description: e.target.value }))}
                  className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{t('task.overview')} *</label>
                <textarea value={createForm.overview} onChange={e => setCreateForm(p => ({ ...p, overview: e.target.value }))} rows={3}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="date" value={createForm.scheduled_date} onChange={e => setCreateForm(p => ({ ...p, scheduled_date: e.target.value }))}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t('task.estimateHours')}</label>
                  <input type="number" min="0" step="0.5" value={createForm.estimate_hours} onChange={e => setCreateForm(p => ({ ...p, estimate_hours: e.target.value }))}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">{t('task.responsible')}</label>
                  <select value={createForm.responsible_user_id} onChange={e => setCreateForm(p => ({ ...p, responsible_user_id: e.target.value }))}
                    className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={() => setShowCreate(false)} className="flex-1 py-2.5 border border-gray-300 text-gray-700 rounded-xl font-medium">{t('common.cancel')}</button>
                <button onClick={handleCreate} disabled={!createForm.short_description || !createForm.overview || saving}
...
function VolunteerCard({ task, isOverdue, currentUserId, users, isAdmin, onManage, onComplete, onTakeResponsibility, onAssign }: any) {
  const { t } = useTranslation();
    ...
    <div className={`rounded-xl shadow-sm border border-gray-100 ${cardBg} overflow-hidden`}>
      <div className="p-4">
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-gray-600 mb-3">
          ...
          <div>👥 {task.crew?.length || 0} crew</div>
