...
def safe_sheet(sheets, idx, label, report):
...
def run_validation(wb, meta_conn, report, layout_version_id=1):
    ...
    report.info(f'Workbook has {num_sheets} sheet(s): {[s.title for s in sheets]}')
    ...
    critical_warn = False
    cur_sheet = None
    missing_sheet_indices = set()

    for block in blocks:
        si = block['sheet_index']

        if si < 0 or si >= num_sheets:
...
def main():
    ...
    try:
        ...
        assessment_id = import_tab1_assessment(ws0, db_conn, args.load_id, report)
